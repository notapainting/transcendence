...
class MessageFirst(BaseSerializer):
    ...
    def validate(self, data):
        if data['target'].get_relation(data['author']) == mod.Relation.Types.BLOCK:
            raise PermissionDenied('you r blocked!', code=403)
        if data['author'] == data['target']:
            raise ValidationError('no pgroup with yourself!', code=400)
        if mod.Group.objects.filter(name='@').filter(members__name=data['author']).filter(members__name=data['target']).exists() is True:
            raise ValidationError('pgroup already exist', code=400)
        return data
...
class MessageRead(BaseSerializer):
    ...
    def validate(self, data):
        ...
        if group.memberships.filter(user=data['author']).exists() is False:
            raise ValidationError("can't send read if not in groupe", code=404)
        return data
...
class MessageFetch(BaseSerializer):
    ...
    def validate(self, data):
        group = data['group']
        if group.memberships.filter(user=data['author']).exists() is False:
            raise PermissionDenied("can't fetch group message if not in", code=403)
        return data
...
class ContactUpdate(BaseSerializer):
    ...
    def validate(self, data):
        if data['author'] == data['name']:
            raise ValidationError('no self contact operation with yourself!', code=400)
        if data['operation'] in [enu.Operations.CONTACT, enu.Operations.INVIT]:
            if data['name'].get_relation(data['author']) is mod.Relation.Types.BLOCK:
                raise PermissionDenied('your blocked', code=403)
        return data
    ...
    def alter_pgroup_if_exist(self, userA, userB, new_role):
        try : 
            ...
            m1 = group.memberships.get(user=userA)
            m2 = group.memberships.get(user=userB)
            m1.role = new_role
            m2.role = new_role
            m1.save()
            m2.save()
            logger.info(f"OP {new_role} - pgroup :{group.id}")
...
class GroupUpdate(BaseSerializer):
    ...
    def validate(self, data):
        group = data['id']
        if group.memberships.filter(user=data['author']).exists() is False:
            raise ValidationError("can't update group if not in", code=400)
        if group.memberships.get(user=data['author']).role < mod.GroupShip.Roles.ADMIN:
            raise PermissionDenied("permission denied", code=403)
        return data
...
class GroupQuit(BaseSerializer):
    ...
    def validate(self, data):
        group = data['group']
        if group.memberships.filter(user=data['author']).exists() is False:
            raise ValidationError("can't quit group if not in", code=400)
        if group.name == '@':
            raise ValidationError("can't quit private conv", code=400)
        return data
